function Gallery() {
	...
	this.init = () => {
		...
		self.swipeDetection(swipeDir => {
			if (swipeDir === 'none') return false;

			let activeItem = document.querySelector('.gallery-item.active');
	...
	this.swipeDetection = callback => {
		let touchsurface = this.bg,
			swipedir,
			startX,
			startY,
			distX,
